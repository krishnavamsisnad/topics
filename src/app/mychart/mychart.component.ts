...
export class MychartComponent implements OnInit {
  chartdata: Saledata[] = []
 saclestotal: { sales: number, order: number, items: number, tax: number }[] = [];
  amountdata:any[]=[]
  labledata:any[]=[]
  colordata:any[]=[]
  salesdata:any[]=[]
        ...
        if (salesReport && salesReport.totals!=null) {
                ...
                this.labledata.forEach((t) => {
                  ...
                  this.saclestotal.push({
                    sales: parseFloat(salesReport.totals[t].sales),
                    order: salesReport.totals['order'] ,
                    items: salesReport.totals['items'],  
                    tax: salesReport.totals['tax'] 
                  });
